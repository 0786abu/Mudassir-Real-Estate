...
<ul className="pagination">
  ...
  {pages.map((p,index) => (
    <li key={index} className={`page-item `}>
      <button style={{background:p===currentPage ? "#108a00" : "",color:p===currentPage ? "white" : "black"}} disabled={p === currentPage || agentloading} className="page-link" onClick={() => goToPage(p)}>{agentloading && p===currentPage ? "...": p}</button>
    </li>
  ))}
