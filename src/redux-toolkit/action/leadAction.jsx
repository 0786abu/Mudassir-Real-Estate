...
export const SendInquiry = (leadData,setLeadData)=>async(dispatch)=>{
    ...
    try {
        const {data} = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/leads`,leadData,{
            headers:{
...
export const MyLLeads = (type)=>async(dispatch)=>{
    ...
    try {
        const {data} = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/leads?type=${type}`,{
            headers:{
                "Content-Type":"application/json"
            },
            withCredentials:true
