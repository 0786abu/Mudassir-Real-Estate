...
const MyListingTab = ({setActiveTab,from,fromTo}) => {
    ...
    <optgroup key={index} label={item.city}>
      {item.subCities.map((sub,index) => (
        ...
        <div>
          ...
          {/* askjdhkashdkjasd */}
            <GridView properties={myProperties} from={from} propertyloading={mypropertyloading} setActiveTab={setActiveTab} fromPanel="user-panel" fromTo={fromTo} />
          </div>
        </div>
        </div>
        )}
